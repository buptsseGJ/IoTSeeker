...
TRAIN_TFRECORD = config.TFRECORD_IOTSEEKER_DIR + os.sep + "train_"+PREFIX+".tfrecord"
TEST_TFRECORD = config.TFRECORD_IOTSEEKER_DIR + os.sep + "test_"+PREFIX+".tfrecord"
VALID_TFRECORD = config.TFRECORD_IOTSEEKER_DIR + os.sep + "valid_"+PREFIX+".tfrecord"
...
# with tf.Session(config=tf.ConfigProto(device_count={'cpu':0})) as sess:
    ...
    sess.run(init_opt)
    if config.SETP5_IF_RESTORE_IOTSEEKER_MODEL:
        saver.restore(sess, config.MODEL_IOTSEEKER_DIR + os.sep + config.STEP5_IOTSEEKER_MODEL_TO_RESTORE)
    ...
    while iter < max_iter:
        ...
        if iter%snapshot == 0:
            ...
            saver.save(sess, config.MODEL_IOTSEEKER_DIR + os.sep + "iotseeker-model"+PREFIX+"_"+str(iter)+".ckpt")
...
# 保存模型
    saver.save(sess, config.MODEL_IOTSEEKER_DIR + os.sep + "iotseeker-model"+PREFIX+"_final.ckpt")
