...
# 计算基本块的非结构性特征
def calBasicBlockFeature_iotseeker(block):
	...
	while curEA <= block.endEA :
		...
		if inst in config_for_feature.IotSeeker_stackInstr:
			StackNum = StackNum + 1
		elif inst in config_for_feature.IotSeeker_arithmeticInstr:
			MathNum = MathNum + 1
		elif inst in config_for_feature.IotSeeker_logicInstr:
			LogicNum = LogicNum + 1
		elif inst in config_for_feature.IotSeeker_compareInstr:
			CompareNum = CompareNum + 1
		elif inst in config_for_feature.IotSeeker_externalInstr:
			ExCallNum = ExCallNum + 1
		elif inst in config_for_feature.IotSeeker_internalInstr:
			InCallNum = InCallNum + 1
		elif inst in config_for_feature.IotSeeker_conditionJumpInstr:
			ConJumpNum = ConJumpNum + 1
		elif inst in config_for_feature.IotSeeker_unconditionJumpInstr:
...
def block_fea(allblock,fea_fp):
	for block in allblock:
		iotseeker_str = calBasicBlockFeature_iotseeker(block)
		fea_str = str(hex(block.startEA)) + "," + iotseeker_str + "\n"
